...
export const MessageBox = ({ message = "", type, onClose, autoClose }: MessageBoxProps) => {
  ...
  const baseClasses =
    "p-4 rounded-lg shadow-md flex items-start gap-3 transition-all duration-300 border";
  ...
  const typeConfig = {
    ...
    warning: {
      ...
      icon: (
        <ExclamationCircleIcon className={`${iconClasses} text-amber-500 dark:text-amber-400`} />
      ),
    },
    info: {
